...
namespace StardewMods
{
    ...
    public class ModEntry : Mod
    {
        ...
        private void GenericModConfigMenuIntegration(object sender, RenderedActiveMenuEventArgs e)     //Generic Mod Config Menu API
        {
            ...
            if (GenericMC != null)
            {
                ...
                try
                {
                    ...
                    GenericMC.AddComplexOption(ModManifest,() => "a", (SpriteBatch b, Vector2 pos) => { }, afterSave: () => UpdateConfig(true));
                }
                catch (Exception)
                {
        ...
        private void UpdateConfig(bool GMCM)
        {
            ...
            if (!GMCM)
            {
                Overlay.locationData = Game1.content.Load<Dictionary<string, LocationData>>("Data\\Locations");       //gets location data (which fish are here)
                Overlay.fishData = Game1.content.Load<Dictionary<string, string>>("Data\\Fish");
